...
class Input extends React.Component{
    ...
    async submit(){
        if(!(this.state.inputCaption&&this.state.nameCaption)){
        ...
        if(this.state.files && this.state.files.length>0){
            await this._uploadFiles().then(async() => {
                ...
                this.record.email = this.props.user.email;
    ...
    render(){
        return(
            <SimpleCard>
                ...
                <AlertBox open={this.state.open} callback={(x)=>{this.setState({open:x})}}>Make sure you have entered name and caption!</AlertBox>
            </SimpleCard>
        )
    }
